  ...
  <div class="main-content" style="padding-top:0px;padding-bottom:0px">
    ...
    <el-row class="mb-2" style="height:38px">
        <el-form-item style="background:#F2F6FC">
          ...
          <el-button v-if="activeType==4" type="default" icon="el-icon-circle-plus-outline" size="mini" @click="forward2ProductAddPage()">新 增</el-button>
          <el-button v-if="activeType==4" type="default" size="mini" @click="exportProduct" icon="el-icon-download">导 出</el-button>
    ...
    >
      ...
      <el-table-column prop="auditState" label="审核状态" width="100px" v-if="activeType==3" align="center" :formatter="formatAuditState"></el-table-column>
      <el-table-column prop="updatedTime" label="修改时间" width="150px" v-if="activeType==3" align="center"></el-table-column>
...
export default {
  ...
  methods: {
    formatSaleState: function (row, column) {
        return row.saleState === 10 ? '在售中' : row.saleState === 11 ? '上架中' : row.saleState === 20 ? '已停售' : row.saleState === 21 ? '下架中' : '--'
    },
